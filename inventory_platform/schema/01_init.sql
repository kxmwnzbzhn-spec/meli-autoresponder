-- Elite Market Inventory Platform — Schema v1.1
-- Supabase PostgreSQL
--
-- v1.1 (2026-05-17) — Sr. Luis Vargas:
--   * Removed DROP TABLE ... CASCADE statements (preserve data on re-migration)
--   * CREATE TABLE → CREATE TABLE IF NOT EXISTS (idempotent)
--   * CREATE INDEX → CREATE INDEX IF NOT EXISTS
--   * CREATE OR REPLACE preserved for views/functions
--   * Trigger uses DROP TRIGGER IF EXISTS guard
--   * No data loss when migration re-runs after Sprint 1+ schema lives alongside
...
CREATE INDEX IF NOT EXISTS idx_products_modelo_color ON products(modelo, color);
...
CREATE INDEX IF NOT EXISTS idx_movements_ts ON stock_movements(ts);
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_stock_no_negative ON stock;
