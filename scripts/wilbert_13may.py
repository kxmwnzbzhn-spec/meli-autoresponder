...
# 13-may CDMX (00:00 a 23:59)
start=datetime(2026,5,13,0,0,0,tzinfo=timezone(timedelta(hours=-6)))
end=datetime(2026,5,13,23,59,59,tzinfo=timezone(timedelta(hours=-6)))
...
print(f"=== {nick} — CIERRE AYER 13/05/2026 (CDMX) ===\n")
...
print(f"\nMargen real: {neto_real/gross*100 if gross else 0:.1f}%")
print(f"\nTOP MODELOS 13-may:")
for t,e in sorted(items_count.items(),key=lambda x:-x[1]["r"])[:8]:
