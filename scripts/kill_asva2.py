...
for IID in ["MLM3849137034","MLM2378087893"]:
  g=requests.get(f"{API}/items/{IID}?attributes=id,title,price,status,available_quantity,sold_quantity",headers=H,timeout=15).json()
  print(f"\n{IID}: {g.get('title','')[:50]} ${g.get('price')} status={g.get('status')} qty={g.get('available_quantity')} sold={g.get('sold_quantity')}")
