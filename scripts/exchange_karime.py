...
CODE="TG-6a4d907631bb0b0001817a7a-3527879962"
...
if r.get("access_token"):
    ...
    print(f"ACCESS_TOKEN_PREFIX={r['access_token'][:30]}",flush=True)
    print(f"REFRESH_TOKEN={r['refresh_token']}",flush=True)
    print(f"EXPIRES_IN={r.get('expires_in')}",flush=True)
    print(f"SCOPE={r.get('scope','?')}",flush=True)
