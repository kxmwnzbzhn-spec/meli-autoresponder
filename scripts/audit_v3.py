...
# Configuración: cuenta, token, ancla, fecha ancla, retiros desde ancla
ACCOUNTS=[
    ...
    ("WILBERT",      "MELI_REFRESH_TOKEN_WILBERT",  338291.62, datetime(2026,5,11,17,20,0,tzinfo=timezone(timedelta(hours=-6))), 31018.00+42797.00),  # retiros 12-may + 13-may
...
print(f"=== AUDITORÍA v3 (fórmula corregida) — {cdmx.strftime('%d-%m-%Y %H:%M')} CDMX ===\n")
...
for label, env, anchor_amt, anchor_dt, retiros in ACCOUNTS:
    ...
    # Pull all orders since 1-april (to capture refunds of older orders that processed post-ancla)
    orders=[]; offset=0
    while True:
    ...
    # Bruto/comisión/envío: solo ventas paid CREADAS después del ancla
    ...
    # Refunds: TODOS los payment.status=refunded con transaction_amount_refunded>0
    # cuya FECHA DEL REFUND (date_last_modified del payment) sea > ancla
    refund=0; refund_count=0
    for o in orders:
    ...
    saldo_pre = anchor_amt + neto_delta
    saldo_final = saldo_pre - retiros
    
    results[label]={"saldo":saldo_final,"anchor":anchor_amt,"gross":gross,"fees":fees,"ship":ship,"refund":refund,"refund_n":refund_count,"retencion":retencion,"retiros":retiros,"paid":qty}
    
    print(f"━ {label}:")
    print(f"   Ancla=${anchor_amt:>10,.2f}  +Bruto=${gross:>10,.2f}  -Com=${fees:>8,.2f}  -Env=${ship:>8,.2f}  -Ref=${refund:>9,.2f}({refund_count})  -Ret={retencion:>7,.2f}")
    print(f"   = Saldo pre-retiro=${saldo_pre:>11,.2f}  -Retiros=${retiros:>10,.2f}  = SALDO=${saldo_final:>11,.2f}\n")

print("\n=== RESUMEN ===")
total=0
for k,v in results.items():
    if "err" in v: continue
    print(f"  {k:<14} ${v['saldo']:>12,.2f}")
    total+=v["saldo"]
print(f"\nTOTAL MELI (excl. Dilcie/Bren/YC_NEW/Mildred): ${total:,.2f}")
