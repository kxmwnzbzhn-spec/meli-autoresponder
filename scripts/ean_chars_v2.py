...
# PASO 1: actualizar solo variations con GTIN (NO tocar item-level attrs por ahora)
new_vars=[]
...
print(f"PASO 1 (EANs): {r.status_code}")
...
# PASO 2: Actualizar item-level attrs sin LINE ni ALPHANUMERIC_MODEL ni los que son not modifiable
ITEM_ATTRS=[
...
r2=requests.put(f"https://api.mercadolibre.com/items/{IID}",headers=H,json={"attributes":ITEM_ATTRS},timeout=30)
print(f"\nPASO 2 (attrs): {r2.status_code}")
if r2.status_code not in (200,201):
    # Procesar y limpiar attrs problematicos
    try: j=r2.json()
    except: j={}
    bad=set()
    for c in j.get("cause",[]):
        code=c.get("code","")
        msg=c.get("message","")
        if "attributes.ignored" in code or "not modifiable" in msg:
            for m_ in re.findall(r"\[([A-Z][A-Z_0-9]+)\]",msg):
                if not m_.startswith("MLM"): bad.add(m_)
        if "attribute.invalid" in code or "number_invalid" in code:
            for m_ in re.findall(r"\[([A-Z][A-Z_0-9]+)\]",msg):
                if not m_.startswith("MLM"): bad.add(m_)
            mm=re.search(r"[Aa]ttribute:?\s+([A-Z][A-Z_]+)",msg)
            if mm: bad.add(mm.group(1))
    print(f"  Bad attrs a quitar: {bad}")
    ITEM_ATTRS2=[a for a in ITEM_ATTRS if a["id"] not in bad]
    r3=requests.put(f"https://api.mercadolibre.com/items/{IID}",headers=H,json={"attributes":ITEM_ATTRS2},timeout=30)
    print(f"  Retry: {r3.status_code}")
    if r3.status_code in (200,201):
        print(f"  OK attrs aplicados: {len(ITEM_ATTRS2)}")
    else:
        print(f"  err: {r3.text[:300]}")
else:
    print(f"  OK attrs aplicados: {len(ITEM_ATTRS)}")
