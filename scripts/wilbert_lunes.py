"""WILBERT etiquetas para imprimir con deadline LUNES 11/MAY/2026 — 4x6 in.
Filtro: status=ready_to_ship, substatus=ready_to_print, deadline en lunes 00:00-23:59 CDMX.
"""
...
# Lunes 11 mayo 2026 CDMX
LUN_START = datetime(2026,5,11,0,0,tzinfo=TZ)
LUN_END   = datetime(2026,5,11,23,59,tzinfo=TZ)
...
def render_header(s, header_h):
    ...
    c.drawCentredString(cx, yellow_top-11, f"[WILBERT] {s['buyer'][:30]} | Ship:{s['sid']}")
...
print(f"=== Wilbert: ventana orders {START.date()} a {NOW.date()} ; deadline LUNES {LUN_START.date()} ===")
at = tok(RT)
...
for sid, ord_o in obs.items():
    try:
        ...
        if st != "ready_to_ship" or sub != "ready_to_print": continue
        # deadline
        deadline=None
        try:
        ...
        if not deadline:
            deadline_count["sin_deadline"] += 1
            continue
        deadline_count[deadline.strftime("%a %d/%m")] += 1
        if deadline < LUN_START or deadline > LUN_END: continue
        ...
        shipments.append({"sid":sid,"buyer":buyer,"comp_lines":comp_lines,
                          "has_used":has_used, "deadline":deadline.strftime("%a %d %H:%M"),
...
print(f"\n=== Distribución deadlines (solo ready_to_print) ===")
for d,n in sorted(deadline_count.items(), key=lambda x:-x[1]): print(f"  {n:4} {d}")
print(f"\n*** Wilbert ready_to_print con deadline LUNES: {len(shipments)} ***")
...
with open("ETIQUETAS_WILBERT_LUNES.pdf","wb") as f: writer.write(f)
print(f"\n✅ PDF: ETIQUETAS_WILBERT_LUNES.pdf ({len(writer.pages)} págs) | Fallidas: {len(fail)}")
