...
def topcat(q):
    try:
        ...
        return cp,(min(pr) if pr else "?")
    except: return "ERR","-"
for label,q in Q:
    try:
        r=requests.get(f"{API}/sites/MLM/search",params={"q":q,"limit":1},headers=H,timeout=20).json()
        total=r.get("paging",{}).get("total",0)
    except Exception as e:
        total=-1
    cp,bb=topcat(q)
    print(f"{label}\t{total}\t{cp}\t{bb}")
    time.sleep(0.2)
print("DONE")
