"""Publica los pendientes como CATALOGO (catalog_listing=True + title).
Asi MELI hereda GTIN del producto de catalogo. Dedup + free shipping."""
...
PEND=["MLM5297098664","MLM2916898793","MLM5297087174","MLM2916908765","MLM2916921607",
...
for sid in PEND:
    try:
        ...
        if not cpid:
            fail.append((sid,"sin_cpid")); print(f"NOCAT {sid}"); continue
        if cpid in have:
            skip.append((sid,cpid)); print(f"SKIP {sid}"); continue
        # CATALOGO TRUE + title (MELI hereda GTIN del catalogo)
        ...
        if r.status_code<300:
            ...
            t=r.text
            code="optin.fake" if "optin.fake" in t else ("title>60" if "title" in t and "60" in t else t[:90])
            fail.append((sid,code)); print(f"FAIL {sid}: {code}")
        time.sleep(0.8)
    except Exception as e:
        fail.append((sid,str(e)[:50])); print(f"ERR {sid}")

print(f"\n=== RESUMEN ===")
print(f"  OK: {len(ok)} | SKIP: {len(skip)} | FAIL: {len(fail)}")
for s,n,st in ok: print(f"  OK {s} → {n} ({st})")
for s,c in fail: print(f"  FAIL {s}: {c}")
