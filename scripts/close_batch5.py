...
ITEMS=["MLM5511027088","MLM3014067117","MLM5510822004","MLM3014080119","MLM5511099772"]
...
for ITEM in ITEMS:
  print(f"\n--- {ITEM} ---")
  g=requests.get(f"{API}/items/{ITEM}",headers={"Authorization":f"Bearer {AT}"},timeout=12)
  if g.status_code!=200:
    print(f"  ❌ GET {g.status_code}: {g.text[:200]}"); continue
  gj=g.json()
  print(f"  [BEFORE] status={gj.get('status')} title={gj.get('title','')[:60]}")
  ...
  rd2=requests.post(f"{SB}/rest/v1/meli_user_directives",headers=SBH,
    json={"account":"AH","scope":"item","scope_value":ITEM,"directive_type":"closed",
          "raw_user_message":"elimina batch 5 items adrian"},timeout=10)
  print(f"  [directive] {rd2.status_code}")
  g2=requests.get(f"{API}/items/{ITEM}",headers={"Authorization":f"Bearer {AT}"},timeout=10)
  if g2.status_code==200:
    print(f"  [AFTER] status={g2.json().get('status')}")
