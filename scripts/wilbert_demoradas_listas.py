"""WILBERT demoradas + listas para enviar = etiquetas para despachar HOY.
Filtro: status=ready_to_ship, substatus IN (printed, delayed). Loggea TODO el panorama.
"""
...
# Demoradas primero, listas después
shipments.sort(key=lambda s: (0 if s["substatus"]=="delayed" else 1,
...
with open("ETIQUETAS_WILBERT_DEMORADAS_LISTAS.pdf","wb") as f: writer.write(f)
print(f"\n✅ PDF: ETIQUETAS_WILBERT_DEMORADAS_LISTAS.pdf ({len(writer.pages)} págs) | Fallidas: {len(fail)}")
