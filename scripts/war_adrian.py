#!/usr/bin/env python3
"""War Adrian v8 — Fix root: SUPABASE_SERVICE_KEY + scope normalization + ceiling/floor directives.
        ...
        try:
            ...
            if pin is not None:
                if float(cur) != pin:
                    ...
                    actions.append({"item_id":iid,"cpid":cpid,"sku":sku,"action_type":"price_change","before_value":str(cur),"after_value":str(pin),"actor":"war_adrian","reason":"user_pin","http_status":r2.status_code,"account":"Adrian"})
                continue
            ...
            if cur>ceil:
                ...
                actions.append({"item_id":iid,"cpid":cpid,"sku":sku,"action_type":"price_change","before_value":str(cur),"after_value":str(ceil),"actor":"war_adrian","reason":f"force_down ceil={ceil}","http_status":r2.status_code,"account":"Adrian"})
                continue
            if cur<floor:
                r2=requests.put(f"{API}/items/{iid}",headers=HJ,json={"price":floor},timeout=12)
                if r2.status_code in (200,201): stat["force_up"]+=1
                actions.append({"item_id":iid,"cpid":cpid,"sku":sku,"action_type":"price_change","before_value":str(cur),"after_value":str(floor),"actor":"war_adrian","reason":f"force_up floor={floor}","http_status":r2.status_code,"account":"Adrian"})
                continue
            ...
            if target is not None and target!=cur:
                ...
                if target!=cur:
                    ...
                    actions.append({"item_id":iid,"cpid":cpid,"sku":sku,"action_type":"price_change","before_value":str(cur),"after_value":str(target),"actor":"war_adrian","reason":reason or pst,"http_status":r2.status_code,"account":"Adrian"})
                else:
                    stat["hold"]+=1
            else:
                stat["hold"]+=1
...
if gh:
    try:
        r=requests.post("https://api.github.com/repos/kxmwnzbzhn-spec/meli-autoresponder/actions/workflows/war_adrian.yml/dispatches",
