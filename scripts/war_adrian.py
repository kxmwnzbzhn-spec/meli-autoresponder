#!/usr/bin/env python3
"""War Adrián loop — long-running self-redispatching.
Cada 60s ejecuta un ciclo completo de war. Corre por 5h30 y se redispatch.
"""
...
while time.time()<end:
    ...
    t0=time.time()
    try:
        scanned,s=war_tick(tick_n)
        print(f"[tick{tick_n} t+{int(t0-start)}s] scanned={scanned} WIN={s['win']} COMP={s['comp']} REI={s['reindex']} ADJ={s['adjusted']} FD={s['force_down']} HOLD={s['hold']} LOCK={s['locked']} ERR={s['err']}")
