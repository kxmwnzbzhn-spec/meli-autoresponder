...
for iid in ["MLM2950801593","MLM2950827337"]:
    g=requests.get(f"{API}/items/{iid}",headers=H,timeout=10).json()
    print(f"{iid}: pre status={g.get('status')} price={g.get('price')} '{(g.get('title') or '')[:38]}'")
    ...
    print(f"  close http={r2.status_code} {('' if r2.status_code<300 else r2.text[:100])}")
    time.sleep(0.3)
