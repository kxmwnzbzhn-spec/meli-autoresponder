...
print(f"{iid}: pre status={g.get('status')} sub={g.get('sub_status')}")
...
print(f"  close http={r.status_code} {('' if r.status_code<300 else r.text[:140])}")
