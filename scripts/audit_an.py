...
print(f"=== AUDITORÍA ANGEL DAMIAN ===")
print(f"Nick: {nick}")
print(f"UID: {uid}")
print(f"Email: {email}")
...
# Saldo MP no es accesible directo, pero podemos estimar
print(f"\n=== SALDO MP ESTIMADO ===")
print(f"NETO 60d (libros) = ${neto:,.2f}")
print(f"+ Ventas pre-60d ya liberadas (cuenta creada 22-nov-2025, varios meses operando)")
print(f"= Saldo MP REAL probablemente MAYOR que NETO 60d (necesita snapshot manual)")
...
print(f"Pausados: {items_p.get('paging',{}).get('total','?')}")
