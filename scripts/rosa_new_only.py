#!/usr/bin/env python3
"""ROSA_MARIA: PDF SOLO con shipments substatus=ready_to_print (nuevas, sin repetir printed)."""
...
OUT="ETIQUETAS_ROSA_MARIA_NUEVAS.pdf"; MANI="MANIFEST_ROSA_MARIA_NUEVAS.json"
...
for sid,rows in by_sid.items():
 ...
 if sh.get("substatus")!="ready_to_print": continue  # SOLO NUEVAS
 items=[]; any_used=False
 for order in rows:
  for line in order.get("order_items") or []:
   obj=line.get("item") or {}; qty=int(line.get("quantity") or 0); iid=obj.get("id") or ""
   if not iid: continue
   at=get_attrs(iid); color=get_color_var(line, at["color"])
 ...
 selected.append({"shipment_id":sid,"items":list(merged.values()),"used":any_used,"multi":sum(x["qty"] for x in merged.values())>=2 and len(merged)>=2,"order_ids":[str(o.get("id")) for o in rows]})
selected.sort(key=lambda x:int(x["shipment_id"]))
print(f"[stats] nuevas ready_to_print: {len(selected)}")
...
def overlay(W,Hh,ship,items,used,multi):
 ...
 c.setFont("Helvetica",7); c.drawCentredString(W/2,y-9,f"[ROSA-MARIA/NUEVA] Ship:{ship}")
