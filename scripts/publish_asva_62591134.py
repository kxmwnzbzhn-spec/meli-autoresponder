...
p=requests.get(f"{API}/products/MLM62591134",headers=H,timeout=10).json()
print(f"Product MLM62591134: '{p.get('name')}' domain={p.get('domain_id')}")

pi=requests.get(f"{API}/products/MLM62591134/items?limit=10",headers=H,timeout=10).json()
...
payload={
    ...
    "condition":"new","catalog_product_id":"MLM62591134","catalog_listing":True,
}
print(f"\nPublicando a $798 (asumido — mismo patrón que los 4 previos)")
