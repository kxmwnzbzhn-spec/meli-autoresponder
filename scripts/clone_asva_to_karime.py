...
AT_L, NEW_RT_L = refresh("MELI_REFRESH_TOKEN_KARIME")
print(f"NEW_RT_ASVA: {NEW_RT_A}",flush=True)
print(f"NEW_RT_KARIME: {NEW_RT_L}",flush=True)
...
if CPID:
    ...
    print(f"\n=== POSTING CATALOG on KARIME cpid={CPID} cat={cat} ===",flush=True)
else:
    # Traditional
    ...
    print(f"\n=== POSTING TRADICIONAL on KARIME cat={cat} attrs={len(new_attrs)} ===",flush=True)

post=requests.post("https://api.mercadolibre.com/items",headers=H_L,json=payload,timeout=25).json()
if "id" in post:
    new_id=post["id"]
    print(f"\n✅ POSTED on KARIME: {new_id} status={post.get('status')} price=${post.get('price')} qty={post.get('available_quantity')}",flush=True)
