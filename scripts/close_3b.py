...
for iid in ["MLM5390346880","MLM5390322498","MLM5390348596"]:
    g=requests.get(f"{API}/items/{iid}",headers=H,timeout=10).json()
    print(f"{iid}: pre status={g.get('status')} sub={g.get('sub_status')} '{(g.get('title') or '')[:38]}'")
    ...
    print(f"  close http={r2.status_code} {('' if r2.status_code<300 else r2.text[:120])}")
    time.sleep(0.3)
