...
# Supabase: directive + no_replenish + replenish_blacklist
...
# meli_user_directives: pause + lock_from_war
for d in [
  {"account":"ASVA","scope":"item","scope_value":ITEM,"directive_type":"pause","raw_user_message":"pausa esto y no lo actives 2967805809 en asva"},
  {"account":"ASVA","scope":"item","scope_value":ITEM,"directive_type":"lock_from_war","raw_user_message":"pausa esto y no lo actives 2967805809 en asva"},
  {"account":"ASVA","scope":"item","scope_value":ITEM,"directive_type":"no_replenish","raw_user_message":"pausa esto y no lo actives 2967805809 en asva"},
]:
...
# meli_no_replenish_items
rn=requests.post(f"{SB}/rest/v1/meli_no_replenish_items",
  headers=SBH,json={"item_id":ITEM,"account":"ASVA","reason":"user: pausa y no actives"},timeout=12)
...
try:
  ...
  os.system("pip install pynacl -q"); import nacl.encoding, nacl.public
...
if GHT and NEW_RT:
  ...
  ru=requests.put(f"https://api.github.com/repos/{R}/actions/secrets/MELI_REFRESH_TOKEN_ASVA",
    headers=GHH,json={"encrypted_value":enc,"key_id":pk["key_id"]},timeout=15)
  print(f"[GH SECRET UPDATE ASVA] HTTP {ru.status_code}")
