...
for src_id in ["MLM3025478283","MLM3018014885"]:
  ...
  # Build TRADICIONAL payload (no catalog_product_id) with GTIN=no aplica + ensure brand/model
  attrs=[]
  has_brand=has_model=has_gtin=False
  for a in src.get("attributes",[]):
    ...
    if aid=="BRAND": has_brand=True
    if aid=="MODEL": has_model=True
  ...
  new_id=j.get("id")
  print(f"  CLONE tradicional {r.status_code} -> {new_id} ${src.get('price')}")
