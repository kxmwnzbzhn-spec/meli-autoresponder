...
print(f"BEFORE: status={g.get('status')} sub={g.get('sub_status')} qty={g.get('available_quantity')} sku=", end="")
attrs=g.get('attributes') or []
for a in attrs:
    if a.get('id')=='SELLER_SKU': print(a.get('value_name'))
r=requests.put(f"{API}/items/{sid}",headers=HJ,json={"status":"paused","available_quantity":0},timeout=30)
print(f"PUT pause+qty0: {r.status_code} {r.text[:160] if r.status_code>=400 else 'OK'}")
