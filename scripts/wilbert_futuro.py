...
# Cutoff: hoy 11:00 AM CDMX. SOLO incluimos deadlines > cutoff (mañana en adelante)
...
for sid, ord_o in obs.items():
    try:
        ...
        # Solo deadline > cutoff (mañana en adelante)
...
with open("ETIQUETAS_WILBERT_FUTURO.pdf","wb") as f: writer.write(f)
print(f"\n✅ PDF: ETIQUETAS_WILBERT_FUTURO.pdf ({len(writer.pages)} págs) | Fallidas: {len(fail)}")
