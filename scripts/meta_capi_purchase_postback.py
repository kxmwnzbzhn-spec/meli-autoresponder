...
PIXEL_ID = "1520455545762550"     # Asva E pixel
LOOKBACK_HOURS = 24
...
def main():
    ...
    print(f"=== Meta CAPI Post-back | seller={EXPECTED_SELLER_ID} | since={since} | whitelist={list(WHITELIST_ITEMS.keys())} ===")

    all_orders = []
    offset = 0
    while True:
        r = requests.get(f"{API_MELI}/orders/search", params={
            "seller": EXPECTED_SELLER_ID, "order.status": "paid",
    ...
    for o, item in target:
        iid = item["item"]["id"]; oid = o["id"]; ot = o["date_created"]
        ...
        if (datetime.now(timezone.utc).timestamp() - event_time) > 7 * 86400:
            print(f"  skip {oid} too old"); continue
        ...
        events.append({
            ...
            "custom_data": {
                ...
                "content_ids": [iid], "content_name": WHITELIST_ITEMS[iid]["name"],
    ...
    if not events:
        print("No valid events. Done."); return

    print(f"Sending {len(events)} events to Meta CAPI...")
    payload = {"data": events}
