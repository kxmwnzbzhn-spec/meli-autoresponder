"""
Etiquetas del día agrupadas por modelo+color, todas las cuentas.
Genera: labels_YYYY-MM-DD/ con un PDF por grupo + manifest.xlsx + summary.txt
"""
...
target_date = sys.argv[1] if len(sys.argv) > 1 else (datetime.now(timezone.utc) - timedelta(hours=6) - timedelta(days=1)).strftime("%Y-%m-%d")
print(f"📅 Etiquetas para {target_date}")

cdmx_offset = timedelta(hours=-6)
day_start = datetime.fromisoformat(target_date).replace(tzinfo=timezone(cdmx_offset))
day_end = day_start + timedelta(days=1)
date_from = day_start.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.000Z")
date_to = day_end.astimezone(timezone.utc).strftime("%Y-%m-%dT%H:%M:%S.000Z")

OUTDIR = f"labels_{target_date}"
os.makedirs(OUTDIR, exist_ok=True)

# Group: { "JBL Go 4|Azul": [(account, shipment_id, order_id, qty, title, buyer), ...] }
...
for label, env_var in ACCOUNTS:
    ...
    print(f"[{label}] {me.get('nickname','')} ({USER_ID})")
    offset = 0; account_orders = 0
    while True:
    ...
    print(f"  → {account_orders} líneas")

print(f"\nTotal: {total_orders} líneas / {total_units} unidades / {len(groups)} grupos\n")

# Download labels grouped — necesitamos token por shipment (el dueño de la cuenta)
# MELI: GET https://api.mercadolibre.com/shipment_labels?shipment_ids=X,Y&response_type=pdf
labels_index = []
for key, items in sorted(groups.items()):
    ...
    # Group by account (mismo token)
    by_account = defaultdict(list)
    for it in items:
        by_account[it["account"]].append(it)
    
    print(f"📦 {model} - {color} ({len(items)} envíos)")
    
    # Combinar PDFs por cuenta y luego concatenar
    all_pdf_bytes = []
    for acct, acct_items in by_account.items():
        ...
        try:
            ...
            print(f"  ❌ {acct}: {e}")
            errors.append({"account":acct,"key":key,"err":str(e)})
    
    # Save combined PDF (concat)
    if all_pdf_bytes:
        ...
        try:
            ...
            # Fallback: solo guardar el primero
...
ws["A1"] = f"📋 Etiquetas {target_date}"
ws["A1"].font = Font(bold=True, size=16, color="1F4E78"); ws.merge_cells("A1:G1")
HEADER = ["Modelo","Color","Envíos","Unidades","Archivo","Cuentas","Items"]
...
for grp in sorted(labels_index, key=lambda x: -x["envios"]):
    ...
    ws.cell(row=r, column=7, value=len(grp["items"]))
    r += 1

# Hoja 2: Detalle
ws2 = wb.create_sheet("Detalle")
HEADER2 = ["Modelo","Color","Cuenta","Order ID","Shipment","Cant","Comprador","Título"]
...
for grp in labels_index:
    for it in grp["items"]:
        ...
        ws2.cell(row=r, column=8, value=it["title"])
        r += 1
...
with open(f"{OUTDIR}/summary.txt", "w") as f:
    f.write(f"Etiquetas {target_date}\n")
    f.write(f"Total envíos: {total_orders} | Unidades: {total_units} | Grupos: {len(labels_index)}\n\n")
    for grp in sorted(labels_index, key=lambda x: -x["envios"]):
        f.write(f"  {grp['model']:25} {grp['color']:15} {grp['envios']:3} envíos / {grp['unidades']:3} unid → {grp['file']}\n")

print(f"\n✅ Generados {len(labels_index)} PDFs en {OUTDIR}/")

if TG_TOKEN and TG_CHAT and total_orders > 0:
    text = f"📦 *Etiquetas {target_date}*\n{total_orders} envíos / {total_units} unidades / {len(labels_index)} grupos\n\nGrupos top:\n"
    for grp in sorted(labels_index, key=lambda x: -x["envios"])[:8]:
        text += f"• {grp['model']} {grp['color']}: {grp['envios']} envíos\n"
    text += f"\nDescargar artifact en GitHub Actions"
