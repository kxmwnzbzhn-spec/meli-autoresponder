...
ALLOWED_SUBS = set(s.strip() for s in os.environ.get("ALLOWED_SUBSTATUSES","ready_to_print").split(",") if s.strip())  # default: solo NUEVAS
# Para regenerar un corte ya emitido, se aceptan únicamente estos envíos exactos.
SHIPMENT_IDS = set(s.strip() for s in os.environ.get("SHIPMENT_IDS", "").split(",") if s.strip())
